...
def call_trajectories(taxi_id=0, date=''):
    try:   
        ...
        cursor.execute(
            "SELECT * FROM trajectories WHERE taxi_id = %s AND date >= %s AND date < %s ORDER BY id ASC LIMIT %s OFFSET %s", 
            (taxi_id, timestamp_str, next_day_timestamp, limit, offset)
        )
        ...
        trajectory_list = [
            Trajectories(db_trajectory[0], db_trajectory[1], db_trajectory[2], db_trajectory[3], db_trajectory[4]) 
            for db_trajectory in db_trajectories
        ]
